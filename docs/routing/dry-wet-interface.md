...
Use instrument/Hi-Z or line inputs according to the interface and pedal manufacturer’s instructions. Keep the monitoring path muted by default; if Safe monitor is explicitly enabled, use headphones and keep the level low while checking the signal.
...
인터페이스와 페달 제조사가 안내한 instrument/Hi-Z 또는 line 입력을 사용하세요. 모니터 경로는 기본적으로 음소거되어 있으며, 안전 모니터링을 명시적으로 켤 때는 헤드폰을 사용하고 레벨을 낮게 유지하세요.
