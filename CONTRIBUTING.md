...
- `develop`은 검토가 끝난 작업을 통합하는 기본 브랜치다.
- 작업은 `feature/*`, `fix/*`, `chore/*` 같은 짧은 브랜치에서 진행한다.
- 자동 검증과 필요한 리뷰가 끝나면 `develop`에 병합한다.
- `main`은 release 전용이며, 수동 Gate와 release 체크가 기록된 경우에만 병합한다.
